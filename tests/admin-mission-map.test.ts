/**
 * Test: GET /api/admin/mission-map
 *
 * Verifies the mission map graph endpoint returns the expected shape
 * and handles tenant-scoped queries correctly.
 */

import { describe, it, expect, beforeAll } from "vitest";
import { getDb } from "../packages/agentos-d/src/db/index.js";
import { createNode, createEdge, getGraph } from "../packages/agentos-d/src/services/mission-map.js";
import type { NodeKind, EdgeKind } from "../packages/agentos-d/src/services/mission-map.js";
...
describe("Mission Map Service", () => {
  ...
  beforeAll(() => {
    ...
    const db = getDb();
    db.deleteFrom("mission_map_edges").where("tenant_id", "=", testTenantId).execute();
    db.deleteFrom("mission_map_nodes").where("tenant_id", "=", testTenantId).execute();
  });
