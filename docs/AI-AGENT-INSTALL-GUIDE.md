...
| `scanner-worker /health unreachable` | Scanner sidecar down, or real embedding mode still downloading model weights. | Fatal by default. Diagnose with `docker compose --env-file ~/.agentworks/config/.env -f ~/.agentworks/source/docker-compose.yml logs scanner-worker --tail 100`. Only use `SMOKE_SCANNER_OPTIONAL=1` for daemon-only debugging. |
| `n8n /healthz unreachable` | n8n down or still booting. | Fatal by default. Wait once if first boot is slow, then diagnose with `docker compose --env-file ~/.agentworks/config/.env -f ~/.agentworks/source/docker-compose.yml logs n8n --tail 100`. Only use `SMOKE_N8N_OPTIONAL=1` for daemon-only debugging. |
| `admin-ui /mission-control unreachable` | Admin UI down or still building/booting. | Fatal by default. Diagnose with `docker compose --env-file ~/.agentworks/config/.env -f ~/.agentworks/source/docker-compose.yml logs admin-ui --tail 100`. Only use `SMOKE_ADMIN_OPTIONAL=1` for daemon-only debugging. |
